...
function ask_for_sudoers_patch 
{
  option=0
  
  read -p "/etc/sudoers rules need to be added in order to make captive portal work. Fix it? (y/n) default (y): " option



  case $option in
	No|no|N|n ) echo "Redirect of captive protal wonn't be working due to missing rules in /etc/sudoers";;
	Yes|yes|Y|y|* ) patch_sudoers;;
  esac
  echo ""
...
#stunnel to make appache portal support https
...
if [ "$ENABLE_CP" == true ]
then
	create_apache_default_conf
	backup_enabled_virtual_hosts
	delete_enabled_virtual_hosts
	enable_cp_site
	create_captive_portal_settings_php
	if ! check_sudoers; then
		#ToDo: use restore_sudoers
		ask_for_sudoers_patch
	fi
	echo "starting webserver for captive-portal"
...
if [ "$ENABLE_CP" == true ]
then
	echo "setup captive portal redirect firewall rules"

	# create chain for captive portal in mangle
	iptables -t mangle -N $portal_chain
	#redirect packets from mangle prerouting to  portal_chain
	iptables -t mangle -A PREROUTING -j $portal_chain
	#mark all packets in mangle portal_chain with 99 (eception rules will be added by captive portal later on)
	iptables -t mangle -i $interface_hotspot -A $portal_chain -j MARK --set-mark 99

	#redirect http requests marked with 99 toh local apache
	iptables -t nat -A PREROUTING -m mark --mark 99 -p tcp --dport 80 -j DNAT --to-destination $interface_hotspot_ip

	#https handled by stunnel
	#iptables -t nat -A PREROUTING -m mark --mark 99 -p tcp --dport 443 -j DNAT --to-destination $interface_hotspot_ip
